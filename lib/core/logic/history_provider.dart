...
class HistoryProvider extends ChangeNotifier {
  ...
  // final Stream<List<OrdersCart>> _listOrderHistory = FirebaseFirestore.instance
  //     .collection('history')
  //     .snapshots()
  //     .map((snapshot) =>
  //         snapshot.docs.map((doc) => OrdersCart.fromJson(doc.data())).toList());

  void changeDoneStatus(OrdersCart toFind, BuildContext context) async {
    ...
    for (QueryDocumentSnapshot doc in querySnapshot.docs) {
      ...
      final orderCart = OrdersCart.fromJson(data);

      if (orderCart.accountInformation.email ==
              toFind.accountInformation.email &&
          orderCart.dateTime == toFind.dateTime) {
        batch.update(
          FirebaseFirestore.instance.collection('history').doc(doc.id),
          {'done_status': true},
        );
      }
    }
    ...
    // final docUser = FirebaseFirestore.instance.collection('history').where({
    //   'account.email': toFind.accountInformation.email,
    //   'date_time': Timestamp.fromDate(DateTime.parse(toFind.dateTime))
    // });
    // docUser.update({'done_status': true});
